...
class Borrowing_model extends CI_Model
{
    ...
    private $columns = [
        0  => 'borrowers.borrower_employee_id',
        1  => 'borrowers.borrower_name',
        2  => 'items.item_name',
        3  => 'items.category',
    ...
    private function _base_query()
{
    ...
    private function _apply_search($search = '')
    {
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like('borrowers.borrowers_employee_|id', $search);
            $this->db->or_like('borrowers.borrower_name', $search);
